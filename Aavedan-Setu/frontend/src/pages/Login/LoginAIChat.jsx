 ...
 * LoginAIChat
 *
 * An interactive, slide-in AI Assistant Chat drawer for the Login page.
 * Mirrors RegisterAIChat behaviour exactly.
 *
 * FUTURE AI INTEGRATION NOTICE:
 * Replace the mock setTimeout inside `handleSendMessageToAI` with an
 * Axios call to your actual Node.js/Python backend LLM route.
 */
export default function LoginAIChat({ isOpen, onClose }) {
  ...
  const handleSendMessageToAI = async (userText) => {
    ...
    try {
      ...
      let replyText =
        "I received your query. In the next phase of this project, I will process this question using an AI model to guide you.";

      if (userText.toLowerCase().includes("complaint")) {
        replyText =
          "To raise a complaint, you'll need to sign up. Once signed up, go to your Dashboard and click 'Submit Complaint'. You can upload images, tag a category (e.g., Waste, Roads), and geo-tag the location.";
      } else if (userText.toLowerCase().includes("scheme")) {
        replyText =
          "Aavedan-Setu offers scheme searches categorized by Farmers, Education, Women Welfare, and Senior Citizens. You can apply directly through the 'Government Schemes' page.";
      } else if (userText.toLowerCase().includes("ayushman")) {
        replyText =
          "Ayushman Bharat scheme provides cover up to Rs 5 Lakhs per family per year for secondary/tertiary hospitalizations. Low-income families listed in SECC database are eligible.";
      } else if (userText.toLowerCase().includes("status")) {
        replyText =
          "You can track any complaint instantly by typing its ID in the 'Track Status' page or inside your dashboard under 'My Complaints'.";
      }
